...
if [ -n "$current" ] then echo -e "${YELLOW}Используется стратегия:${NC}  ${CYAN}$current $RKN_STATUS $( [ -n "$DV" ] && echo "$DV" )${NC}"; elif [ -n "$RKN_STATUS" ]; then echo -e "${YELLOW}Используется стратегия:${NC}${CYAN}  РКН${NC} $( [ -n "$DV" ] && echo "$DV" )"; fi; fi
