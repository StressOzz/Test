...
CACHE_VER="/tmp/zapret_version"; LATEST_VER=$(curl -s https://github.com/remittor/zapret-openwrt/releases | tr '\n' ' ' | grep -oE 'zapret v[0-9]+\.[0-9]+' | head -1 | sed -E 's/zapret v//')
if [ ! -f "$CACHE_VER" ] || [ "$(cat $CACHE_VER)" != "$LATEST_VER" ]; then echo "$LATEST_VER" > "$CACHE_VER"; fi; ZAPRET_VERSION=$(cat "$CACHE_VER")
