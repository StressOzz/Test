...
"kinozal.tv|https://kinozal.tv" "cub.red|https://cub.red" "ottai.com|https://ottai.com" >> "$OUT_DPI"; curl -fsSL "$RAW" | sed -n 's/.*"url":[[:space:]]*"\([^"]*\)".*/\1/p' | while read u; do echo "$(basename "$u")|$u"; done >> "$OUT_DPI" \
