...
if [ -f /etc/init.d/zapret ] && [ -f "$CONF" ]; then
    if grep -q "option DISABLE_IPV6 '1'" "$CONF"; then
        echo -е "${CYAN}7) ${GREEN}Включить ${NC}IPv6${GREEN} в ${NC}Zapret"
    else
        echo -е "${CYAN}7) ${GREEN}Выключить ${NC}IPv6${GREEN} в ${NC}Zapret"
    fi
fi
...
toggle_ipv6() {
    ...
    if grep -q "option DISABLE_IPV6 '0'" "$CONF"; then
        ...
        echo -e "\n${MAGENTA}Выключаем IPv6 в Zapret${NC}"
        ZAPRET_RESTART
        echo -e "IPv6${GREEN} выключен в ${NC}Zapret\n"
        PAUSE
    fi
    ...
    if [ -z "$time" ]; then
        echo -e "Google IPv6: ${RED}FAIL${NC}"
        echo -e "${RED}IPv6 недоступен! Включение не рекомендуется!${NC}\n"
        PAUSE
        return
    fi

    echo -e "Google IPv6: ${GREEN}OK ($time ms)${NC}\n"
    
    sed -i "s/option DISABLE_IPV6 '1'/option DISABLE_IPV6 '0'/" "$CONF"
    echo -e "\n${MAGENTA}Включаем IPv6 в Zapret"
    ZAPRET_RESTART
    echo -e "IPv6${GREEN} включен в ${NC}Zapret\n"
    PAUSE
}
