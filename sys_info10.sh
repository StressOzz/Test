...
echo -e "\n${GREEN}===== Flow Offloading =====${NC}"; sw=$(uci -q get firewall.@defaults[0].flow_offloading); hw=$(uci -q get firewall.@defaults[0].flow_offloading_hw); if grep -q 'ct original packets ge 30' /usr/share/firewall4/templates/ruleset.uc 2>/dev/null; then
dpi="${RED}yes${NC}"; else dpi="${GREEN}no${NC}"; fi; if [ "$hw" = "1" ]; then out="HW: ${RED}on${NC}"; elif [ "$sw" = "1" ]; then out="SW: ${RED}on${NC}"; else out="SW: ${GREEN}off${NC} | HW: ${GREEN}off${NC}"; fi
out="$out | FIX: ${dpi}"; echo -e "$out\n\n${GREEN}===== Проверка GitHub =====${NC}"; RATE=$(curl -s https://api.github.com/rate_limit | grep '"remaining"' | head -1 | awk '{print $2}' | tr -d ,)
...
sites_clean="$SITES"; total=$(echo "$sites_clean" | wc -w); half=$(( (total + 1) / 2 )); sites_list="$sites_clean"; for idx in $(seq 1 $half); do left=$(echo $sites_list | cut -d' ' -f$idx); right_idx=$((idx + half))
right=$(echo $sites_list | cut -d' ' -f$right_idx); left_pad=$(printf "%-25s" "$left"); right_pad=$(printf "%-25s" "$right")
if curl -Is --connect-timeout 3 --max-time 4 "https://$left" >/dev/null 2>&1; then left_color="[${GREEN}OK${NC}]  "; else left_color="[${RED}FAIL${NC}]"; fi
if [ -n "$right" ]; then if curl -Is --connect-timeout 3 --max-time 4 "https://$right" >/dev/null 2>&1; then right_color="[${GREEN}OK${NC}]  "; else right_color="[${RED}FAIL${NC}]"; fi
echo -e "$left_color $left_pad $right_color $right_pad"; else echo -e "$left_color $left_pad"; fi; done ; echo; read -p "Нажмите Enter для выхода в главное меню..." dummy; }
# ==========================================
# Главное меню
# ==========================================
show_menu() {
...
5) uninstall_zapret;; 6) fix_GAME  ;; 7) enable_discord_calls ;; 8) zapret_key ;; 9) sys_info ;; *) echo; exit 0 ;; esac; }
# ==========================================
# Старт скрипта
# ==========================================
