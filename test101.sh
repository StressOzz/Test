...
get_ip4() {
    SERVER="${2%%#*}" # убираем #порт если есть
    nslookup -type=A "$1" "$SERVER" 2>/dev/null | awk '/^Address: /{print $2}' | grep -E '^[0-9.]+' | tail -n1
}
...
for DOMAIN in $DOMAINS; do
    ...
    if [ -z "$SYS_IP" ]; then
        ...
        DNS_COLOR=$RED
        FINAL_DNS_OK=0
    elif [ $MATCH -eq $TOTAL ] && [ $TOTAL -gt 0 ]; then
        DNS_RESULT="OK"
        DNS_COLOR=$GREEN
    else
    ...
    # --- DPI проверка ---
    if [ -n "$SYS_IP" ]; then
        curl -m 5 -I --silent --resolve "$DOMAIN:443:$SYS_IP" "https://$DOMAIN" >/dev/null 2>&1
        if [ $? -eq 0 ]; then
            DPI_RESULT="OK"
            DPI_COLOR=$GREEN
        else
