...
# =========================================
ZAPRET_MANAGER_VERSION="9.78"; STR_VERSION_AUTOINSTALL="v7"
...
ZAPRET_VERSION="72.20260307"; PODKOP_LATEST_VER="0.9.6"; GO_VER="0.9.2"; MT_VERSION="0.8.2"
...
DIC_FAKE() {
	if ! grep -q -- "--filter-l7=discord,stun" "$CONF" || ! grep -q -- "--dpi-desync=fake" "$CONF"; then
...
' "$CONF"
	...
	echo -e "${CYAN}1) ${GREEN}Сменить на ${NC}stun.bin $(stat_f stun.bin)"
	echo -e "${CYAN}2) ${GREEN}Сменить на ${NC}stun2.bin $(stat_f stun2.bin)"
	echo -e "${CYAN}3) ${GREEN}Сменить на ${NC}quic_initial_www_google_com.bin $(stat_f quic_initial_www_google_com.bin)"
	...
	case "$choiceF" in
		1) new_fileD="stun.bin";;
		2) new_fileD="stun2.bin";;
		3) new_fileD="quic_initial_www_google_com.bin";;
		4) new_fileD="quic_initial_dbankcloud_ru.bin";;
		5) new_fileD="quic_initial_steamcommunity_com.bin";;
		6) new_fileD="quic_initial_tencent_com.bin";;
		7) new_fileD="DELETE";;
		*) return;;
	esac
...
' "$CONF" > "$CONF.tmp" && mv "$CONF.tmp" "$CONF"
	...
	if [ "$new_fileD" = "DELETE" ]; then
		echo -e "fake ${GREEN}удалён!${NC}\n"
	else
		echo -e "fake ${GREEN}изменён на ${NC}${new_fileD}${GREEN}!${NC}\n"
	fi
...
read -r choiceF; case "$choiceF" in 1) new_file="stun.bin";; 2) new_file="stun2.bin";; 3) new_file="quic_initial_4pda.to.bin";; 4) new_file="quic_initial_dbankcloud_ru.bin";; 5) new_file="quic_initial_steamcommunity_com.bin";; 6) new_file="quic_initial_tencent_com.bin";; *) return;; esac; echo -e "\n${CYAN}Устанавливаем fake${NC} ${new_file}"
...
"kinozal.tv|https://kinozal.tv" "anitokyo.tv|https://anitokyo.tv" "cub.red|https://cub.red" >> "$OUT_DPI"; curl -fsSL "$RAW" | sed -n 's/.*"id":[[:space:]]*"\([^"]*\)".*"host":[[:space:]]*"\([^"]*\)".*/\1|\2/p' >> "$OUT_DPI" \
