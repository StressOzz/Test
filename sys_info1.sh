...
PKGS=$(awk '/^Package:/ {p=$2} /^Status: install user/ {print p}' /usr/lib/opkg/status); total=$(echo "$PKGS" | wc -l); half=$(( (total + 1) / 2 ))
...
if [ "$hw" = "1" ]; then out="HW: ${RED}on${NC}"; elif [ "$sw" = "1" ]; then out="SW: ${RED}on${NC}"; else out="SW: ${GREEN}off${NC} | HW: ${GREEN}off${NC}"; fi
...
ZAPRET_STATUS="${RED}остановлен${NC}"; fi
SCRIPT_FILE="/opt/zapret/init.d/openwrt/custom.d/50-script.sh"; if [ -f "$SCRIPT_FILE" ]; then line=$(head -n1 "$SCRIPT_FILE")
case "$line" in
*QUIC*) name="50-quic4all" ;; *stun*) name="50-stun4all" ;; *"discord media"*) name="50-discord-media" ;; *"discord subnets"*) name="50-discord" ;; *) name="" ;;
esac
fi
...
if [ -f /etc/init.d/zapret ]; then zpr_info
else
...
left=$(echo $sites_list | cut -d' ' -f$idx); right_idx=$((idx + half)); right=$(echo $sites_list | cut -d' ' -f$right_idx); left_pad=$(printf "%-25s" "$left")
right_pad=$(printf "%-25s" "$right")
if curl -Is --connect-timeout 3 --max-time 4 "https://$left" >/dev/null 2>&1; then left_color="[${GREEN}OK${NC}]  "
else
left_color="[${RED}FAIL${NC}]"; fi; if [ -n "$right" ]; then if curl -Is --connect-timeout 3 --max-time 4 "https://$right" >/dev/null 2>&1; then right_color="[${GREEN}OK${NC}]  "
else
right_color="[${RED}FAIL${NC}]"; fi
echo -e "$left_color $left_pad $right_color $right_pad"
else
echo -e "$left_color $left_pad"; fi; done
