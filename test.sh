...
ZAPRET_VERSION="72.20260307"; PODKOP_LATEST_VER="0.9.2"; GO_VER="0.9.2"; MT_VERSION="0.8.0"
...
get_ver "https://github.com/MagiTrickle/MagiTrickle/releases/latest" "$TMP_MAG_VER" "MagiTrickle" & get_ver "https://github.com/remittor/zapret-openwrt/releases/latest" "$TMP_VER" "Zapret" & get_ver "https://github.com/spatiumstas/tg-ws-proxy-go/releases/latest" "$TMP_VER_GO" "TG-WS Proxy GO" &
wait; [ -s "$TMP_VER" ] && ZAPRET_VERSION="$(cat "$TMP_VER")"; [ -s "$TMP_VER_GO" ] && GO_VER="$(cat "$TMP_VER_GO")"; [ -s "$TMP_MAG_VER" ] && MT_VERSION="$(cat "$TMP_MAG_VER")"
...
if [ "$AVAILABLE_SPACE" -lt "$REQUIRED_SPACE" ]; then
    ...
    if [ "$ACTION" = "update" ]; then
        ...
        case "$answer" in
            y|Y|д|Д)
                echo -e "\n${CYAN}Удаляем старую версию NetShift${NC}"
...
pkg_is_installed netshift && { [ "$INST_VER_POD" != "$PODKOP_LATEST_VER" ] && echo -e "${YELLOW}NetShift:${NC}            ${RED}$VER_POD (версия устарела)${NC}" || echo -e "${YELLOW}NetShift:${NC}            ${GREEN}$VER_POD${NC}"; }
