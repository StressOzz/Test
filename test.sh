...
start_service() {
    procd_open_instance
    procd_set_param command $BIN --host 0.0.0.0 $PORT $SECRET_ARG
...
install_TG_RS() {
    ...
    create_init "tg-ws-proxy-rs" "/usr/bin/tg-ws-proxy-rs" "--port 2443" "--secret $SECRET"
...
create_init "tg-ws-proxy" "/usr/bin/tg-ws-proxy" "" "--secret $SECRET"
...
install_TG_GO() {
    ...
    create_init "tg-ws-proxy-go" "/usr/bin/tg-ws-proxy-go" "--port 1080" ""
...
if pgrep -f tg-ws-proxy >/dev/null 2>&1 && [ -f "$BIN_PATH_PH" ] && [ -f "$INIT_PATH_PH" ]; then
    ...
    echo -e "${YELLOW}Ссылка для подключения:${NC}\ntg://proxy?server=$(ip -4 route get 1 | awk '{print $7; exit}')&port=1443&secret=dd$SECRET_IN_PH"
fi
