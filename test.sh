...
check_mirror() {
URL="$1"
if curl -sL --connect-timeout 2 --max-time 3 --speed-time 3 --speed-limit 1 --range 0-65535 -A "Mozilla/5.0 (Windows NT 10.0; Win64; x64) curl/8.0" -o /dev/null "$URL" >/dev/null 2>&1; then
echo -e "${GREEN}OK${NC}"
else
echo -e "${RED}FAIL${NC}"
fi
...
S1=$(check_mirror "https://mirror.sjtu.edu.cn/openwrt/releases/")
S2=$(check_mirror "https://mirror.berlin.freifunk.net/downloads.openwrt.org/releases/")
S3=$(check_mirror "https://mirror.tiguinet.net/openwrt/releases/")
S4=$(check_mirror "https://mirror.ps.kz/openwrt/releases/")
S5=$(check_mirror "https://ftp.snt.utwente.nl/pub/software/openwrt/releases/")
S6=$(check_mirror "https://downloads.openwrt.org/releases/")

echo -e "${MAGENTA}Меню выбора зеркала OpenWrt${NC}\n"
echo -e "${YELLOW}Используется зеркало: ${GREEN}$CURR${NC}\n"

echo -e "${CYAN}1)${NC} China ${S1}"
echo -e "${CYAN}2)${NC} Germany ${S2}"
echo -e "${CYAN}3)${NC} Belgium ${S3}"
echo -e "${CYAN}4)${NC} Kazakhstan ${S4}"
echo -e "${CYAN}5)${NC} Netherlands ${S5}"
echo -e "${CYAN}6)${NC} default / OpenWrt ${S6}"
echo -e "${CYAN}Enter)${GREEN} Выход в системное меню${NC}"

echo -en "\n${YELLOW}Выберите зеркало: ${NC}"
read -r z

case "$z" in
1) set_mirror "mirror.sjtu.edu.cn/openwrt" ;;
2) set_mirror "mirror.berlin.freifunk.net/downloads.openwrt.org" ;;
3) set_mirror "mirror.tiguinet.net/openwrt" ;;
4) set_mirror "mirror.ps.kz/openwrt" ;;
5) set_mirror "ftp.snt.utwente.nl/pub/software/openwrt" ;;
6) set_mirror "downloads.openwrt.org" ;;
*) break ;;
esac
