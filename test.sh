...
echo -e "${CYAN}7) ${WSSIZE_MENU_TEXT}${NC}"; echo -e "${CYAN}8) ${methodeol_MENU_TEXT}${NC}"; if grep -q "^#udp443" $CONF; then echo -e "${CYAN}9) ${GREEN}Удалить блок ${NC}--filter-udp=443"; else echo "${CYAN}9) ${GREEN}Добавить блок ${NC}--filter-udp=443"; fi
...
ZAPRET_RESTART; echo -e "${GREEN}Блок с ${NC}--filter-udp=443${GREEN} добавлен!${NC}"; fi ;; *) return;; esac; done }
