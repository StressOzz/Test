...
BASE="https://github.com/routerich/packages.routerich"
BRANCH="24.10.6"
BASE_HTML="$BASE/tree/$BRANCH/routerich"
RAW_BASE="$BASE/raw/refs/heads/$BRANCH/routerich"
TMP="/tmp/z2r"
...
find_latest() {
    wget -qO- "$BASE_HTML" | grep -oE "$1[^\"']+\.ipk" | sort -Vr | head -n1
}
...
install_pkg() {
    ...
    wget -q --show-progress "$RAW_BASE/$PKG" -O "$TMP/$PKG" || {
...
is_routerich_added() {
    [ -f /etc/opkg/customfeeds.conf ] && grep -q "^src/gz routerich " /etc/opkg/customfeeds.conf
}

install_zapret() {
    opkg update

    install_pkg "zapret2_" || return
    install_pkg "luci-app-zapret2_" || return
...
install_zeroblock() {
    ...
    install_pkg "zeroblock" || return
    install_pkg "luci-app-zeroblock" || return
...
add_routerich() {
    ...
    if ! grep -q "^src/gz routerich " "$FILE"; then
        echo "src/gz routerich $BASE/raw/$BRANCH/routerich" >> "$FILE"
    fi
