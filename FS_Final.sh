...
while true; do
    ...
    echo ""
    printf "Выберите стратегию (1-%s, q-выход): " "$COUNT"
    read SEL

    [ "$SEL" = "q" ] && { echo -e "${YELLOW}Выход...${NC}"; exit 0; }
    ...
    [ "$SEL" -lt 1 ] || [ "$SEL" -gt "$COUNT" ] && { echo -e "${RED}Номер вне диапазона${NC}"; continue; }
    ...
    # Сохраняем выбранную стратегию во временный файл
