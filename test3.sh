...
strategy_v10() { printf '%s\n' "#v10" "--filter-tcp=443" "--dpi-desync=split2" "--dpi-desync-split-seqovl=681" "--dpi-desync-split-seqovl-pattern=/opt/zapret/files/fake/stun.bin"; }
# ==========================================
# Cтратегии Flowseal
# ==========================================
...
strategy_CHOUSE () { echo -ne "\n${YELLOW}Введите версию стратегии ${NC}(1-10)${YELLOW}:${NC} "; read -r choice; if [[ "$choice" =~ ^[1-10]$ ]]; then install_strategy "v$choice"; fi; }
...
read CHOICE </dev/tty; if ! echo "$CHOICE" | grep -qE '^[0-10]+$' || [ "$CHOICE" -lt 1 ] || [ "$CHOICE" -gt "$COUNT" ]; then return 1; fi; SELECTED_NAME=$(sed -n "${CHOICE}p" $TMP_SF/strategy_list); rm -f $TMP_SF/strategy_list
