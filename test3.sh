...
name=$(case "$line" in *QUIC*) echo "50-quic4all" ;; *stun*) echo "50-stun4all" ;; *"discord media"*) echo "50-discord-media" ;; *"discord subnets"*) echo "50-discord" ;; *50-discord-addIP*) echo "50-discord-addIP" ;; *) echo "" ;; esac); }
...
5) SELECTED="50-discord-addIP"; URL="https://raw.githubusercontent.com/StressOzz/Test/refs/heads/main/50-discord-addIP" ;;
...
fi; fi; menu_game=$( [ -f "$CONF" ] && grep -q "1024-19293,19345-49999,50101-65535" "$CONF" && echo "Удалить стратегию для игр" || echo "Добавить стратегию для игр" ); pgrep -f "/opt/zapret" && str_stp_zpr="Остановить" || str_stp_zpr="Запустить"
