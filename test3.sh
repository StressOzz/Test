...
hosts_enabled() { if grep -q "#malw.link" /etc/hosts; then hosts_echo="Malw.link"; return 0; elif grep -q "#mafioznik" /etc/hosts; then hosts_echo="Mafioznik"; return 0; elif grep -q "### dns.geohide.ru" /etc/hosts; then hosts_echo="GeoHide"; return 0
...
echo -e "\n${GREEN}===== Настройки hostst =====${NC}"; if hosts_enabled; then echo -e "Домены в hosts: ${GREEN}$hosts_echo${NC}"; fi
