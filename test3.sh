...
command -v curl >/dev/null 2>&1 || { echo -e "${CYAN}Устанавливаем ${NC}curl"; $([ "$PKG_IS_APK" -eq 1 ] && echo apk update >/dev/null 2>&1 && echo apk add curl >/dev/null 2>&1 || echo opkg update >/dev/null 2>&1 && echo opkg install curl >/dev/null 2>&1) || { echo -e "\n${RED}Ошибка установки curl${NC}"; PAUSE; return 1; }; }
