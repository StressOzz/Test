...
install_TG_RS() {
    ...
    tar -xzf "$TMP_ARCHIVE_RS" -C "$TMP_DIR_RS" || { echo -e "\n${RED}Ошибка распаковки${NC}"; rm -rf "$TMP_ARCHIVE_RS"; PAUSE; return 1; }

    BIN_FILE="$(find "$TMP_DIR_RS" -type f -name tg-ws-proxy | head -n1)"

    [ -n "$BIN_FILE" ] || { echo -e "\n${RED}файл tg-ws-proxy не найден!${NC}"; rm -rf "$TMP_DIR_RS" "$TMP_ARCHIVE_RS"; PAUSE; return 1; }

    rm -f "$BIN_PATH_RS"
    mv "$BIN_FILE" "$BIN_PATH_RS" || { echo -e "\n${RED}Ошибка перемещения файла!${NC}"; rm -rf "$TMP_DIR_RS" "$TMP_ARCHIVE_RS"; PAUSE; return 1; }
