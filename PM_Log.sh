...
install_pkg "luci-i18n-amneziawg-ru" || echo -e "${RED}Внимание: русская локализация не установлена (не критично)${NC}"
rm -rf "$AWG_DIR"
...
get_versions() {
 ...
 # --- ByeDPI установленная версия ---
    ...
    if command -v podkop; then
        PODKOP_VER=$(podkop show_version | sed 's/-r[0-9]\+$//')
...
install_ByeDPI() {
    ...
    opkg install --force-reinstall "$BYEDPI_FILE"
...
opkg remove --force-removal-of-dependent-packages byedpi
uci delete dhcp.@dnsmasq[0].localuse; uci commit dhcp; /etc/init.d/dnsmasq restart
...
install_podkop() {
    ...
    # Проверка системы
    MODEL=$(cat /tmp/sysinfo/model || echo "не определено")
...
integration_byedpi_podkop() {
    ...
    if ! command -v byedpi && [ ! -f /etc/init.d/byedpi ]; then
...
uninstall_podkop() {
    ...
    # Удаляем пакеты
    opkg remove luci-i18n-podkop-ru luci-app-podkop podkop --autoremove || true
...
opkg remove --force-removal-of-dependent-packages luci-i18n-amneziawg-ru luci-proto-amneziawg amneziawg-tools kmod-amneziawg
...
if command -v amneziawg || opkg list-installed | grep -q "^amneziawg-tools"; then
    echo -e "${YELLOW}AWG: ${GREEN}установлен${NC}"
else
    echo -e "${YELLOW}AWG: ${RED}не установлен${NC}"
fi

if uci -q get network.AWG ; then
