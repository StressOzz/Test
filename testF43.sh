...
if grep -q -- "--hostlist-exclude=/opt/zapret/ipset/zapret-hosts-user-exclude.txt" "$CONF" && grep -qE "#v[1-99]" "$CONF"; then
    enable_rkn
    read -p "Нажмите Enter..." dummy </dev/tty
...
sed -i "s|^\([[:space:]]*option NFQWS_PORTS_UDP '\).*'|\1443,1024-65535'|" "$CONF"
