...
echo -e "\n${YELLOW}Версия:    ${INST_COLOR}$INSTALLED_DISPLAY${NC}"
[ -n "$ZAPRET_STATUS" ] && echo -e "${YELLOW}Статус:${NC}    $ZAPRET_STATUS"
show_script_50 && [ -n "$name" ] && echo -e "${YELLOW}Скрипт:${NC}    $name"
[ -f "$CONF" ] && grep -q "option NFQWS_PORTS_UDP.*1024-49999,50100-65535" "$CONF" && grep -q -- "--filter-udp=1024-49999,50100-65535" "$CONF" && echo -e "${YELLOW}Игры:${NC}      ${GREEN}активированы${NC}"
show_current_strategy && [ -n "$ver" ] && echo -e "${YELLOW}Стратегия:${NC} ${CYAN}$ver${NC}"
