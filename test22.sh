...
# curl -sL -o /dev/null -w '%{url_effective}' https://github.com/spatiumstas/tg-ws-proxy-go/releases/latest | grep -o '[0-9.]*$' > "$TMP_VER_GO"; GO_VER="$(cat "$TMP_VER_GO")"
...
get_ver() {
  ...
  RESULT=$(curl -sL --connect-timeout 5 --max-time 15 --retry 2 --retry-delay 2 \
  ...
  if [ $CURL_EXIT -ne 0 ]; then
    echo "✗ $NAME: ошибка curl (код $CURL_EXIT)"
    echo "0.0.0" > "$OUT_FILE"
    return 1
  fi
  ...
  if [ -z "$VERSION" ]; then
    echo "✗ $NAME: не удалось извлечь версию (HTTP $HTTP_CODE)"
    echo "URL: $FINAL_URL"
    echo "0.0.0" > "$OUT_FILE"
    return 1
  fi
...
TGSTATUS=""
pidof tg-ws-proxy-go >/dev/null 2>&1 && TGSTATUS="${TGSTATUS:+$TGSTATUS/}Go SOCKS5"
pidof tg-ws-proxy >/dev/null 2>&1 && TGSTATUS="${TGSTATUS:+$TGSTATUS/}Go MTProto"
...
if [ -n "$INSTALLED_VER_GO" ]; then
    if [ "$GO_ACTION" = "update" ]; then
        echo -e "${YELLOW}TG WS Proxy Go MTProto версия:${NC} ${RED}$INSTALLED_VER_GO (есть обновление → $GO_VER)${NC}"
    else
...
if pidof tg-ws-proxy >/dev/null 2>&1 && [ -f "/etc/init.d/tg-ws-proxy" ]; then
   ...
   echo -e "\n${YELLOW}Настройки ${CYAN}TG WS Proxy Go MTProto${YELLOW}:${NC}"
...
echo -ne "${CYAN}3)${GREEN} "
if [ "$GO_ACTION" = "install" ]; then
    echo -e "Установить ${NC}TG WS Proxy Go MTProto"
elif [ "$GO_ACTION" = "update" ]; then
    echo -e "Обновить ${NC}TG WS Proxy Go MTProto${NC}"
else
    echo -e "Удалить ${NC}TG WS Proxy Go MTProto${NC}"
fi
...
3)
    if [ "$GO_ACTION" = "install" ] || [ "$GO_ACTION" = "update" ]; then
        install_update_TG_PKG
    else
        remove_TG_PKG
    fi
