...
4) stop_zapret "1"; echo -e "n${YELLOW}${NC}blockcheck ${YELLOW} можно прервать - ${NC}Ctrl+C${YELLOW}, при этом ${NC}Zapret ${YELLOW}останется ${NC}остановленным !"; echo; read -p "Нажмите Enter для запуска blockcheck..." dummy; clear; chmod +x /opt/zapret/blockcheck.sh; /opt/zapret/blockcheck.sh; start_zapret ;;
