...
echo -e "${CYAN}2) ${GREEN}Меню ${NC}TG WS Proxy\n${CYAN}3) ${GREEN}Меню стратегий${NC}"
...
1) $Z_ACTION_FUNC;; 2) menu_TG;; 3) menu_str;; 4)TEST_menu;; 5) DoH_menu;; 6) Discord_menu;; 7) menu_hosts;; 8) zapret_key;; 9) pgrep -f /opt/zapret >/dev/null 2>&1 && stop_zapret || start_zapret;; 0) sys_menu;; *) echo; exit 0;; esac; }; while true; do show_menu; done
