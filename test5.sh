...
ZAPRET_VERSION="72.20260307"; PODKOP_LATEST_VER="0.8.8"; GO_VER="0.7.2"
...
DELETE="opkg remove --autoremove --force-removal-of-dependent-packages"; ARCH="$(opkg print-architecture | awk '{print $2}' | tail -n1)"; VER_SUF="r1-all"; INST_POD=$(opkg list-installed "$PKG" 2>/dev/null | awk '{print $3}')
APK_RAS="ipk"; SUFICS="v"; TMP_FILE_GO="/tmp/tg-ws-proxy.ipk"; else PKG="apk"; GO_SUF="r1"; CONFZ="/etc/apk/repositories.d/distfeeds.list"; PKG_IS_APK=1; INST_POD=$(apk info "$PKG" 2>/dev/null | sed -n 's/.*-\([0-9].*\)/\1/p')
...
pkg_is_installed netshift && { INST_VER_POD="$INST_POD"; [ "$INST_VER_POD" != "$PODKOP_LATEST_VER" ] && echo -e "${YELLOW}NetShift:${NC}                ${RED}версия устарела${NC}" || echo -e "${YELLOW}NetShift:${NC}                ${GREEN}установлен${NC}"; }
