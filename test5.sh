...
zpr_info() { if [ "$PKG_IS_APK" -eq 1 ]; then INSTALLED_VER=$(apk info zapret | awk '/^zapret-[0-9]/ {sub(/^zapret-/, ""); sub(/-r[0-9]+$/, ""); print; exit}')
else INSTALLED_VER=$(opkg list-installed | grep '^zapret ' | awk '{print $3}'); fi
