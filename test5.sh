...
echo -n "Google IPv4: "
time=$(ping -4 -c 1 -W 2 google.com 2>/dev/null | grep 'time=' | awk -F'time=' '{print $2}' | awk '{print $1}')
if [ -n "$time" ]; then
...
# Yandex IPv4
echo -n "Yandex IPv4: "
time=$(ping -4 -c 1 -W 2 yandex.ru 2>/dev/null | grep 'time=' | awk -F'time=' '{print $2}' | awk '{print $1}')
if [ -n "$time" ]; then
    echo -e "${GREEN}ok ($time ms)${NC}"
else
    echo -e "${RED}fail${NC}"
