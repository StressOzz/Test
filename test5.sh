...
AUTO_STRATEGY_MODE_FILE="/etc/zapret_auto_strategy_mode"

get_auto_strategy_mode() {
    [ -f "$AUTO_STRATEGY_MODE_FILE" ] && cat "$AUTO_STRATEGY_MODE_FILE" || echo "3"
}
...
case "$MODE" in
    1) MODE_NAME="только v" ;;
    2) MODE_NAME="только Flowseal" ;;
    *) MODE_NAME="v + Flowseal" ;;
esac

echo -e "${YELLOW}Стратегии для тестирования:${NC} ${GREEN}$MODE_NAME${NC}"
