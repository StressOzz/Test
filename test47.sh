...
# ==========================================
ZAPRET_MANAGER_VERSION="7.3"; ZAPRET_VERSION="72.20251312"; STR_VERSION_AUTOINSTALL="v5"
...
chmod +x /usr/bin/zms; echo -e "${CYAN}Обновляем список пакетов${NC}"
...
read -p "Нажмите Enter..." dummy; return; fi
echo -e "${CYAN}Устанавливаем ${NC}luci-app-ttyd"; if ! opkg install luci-app-ttyd >/dev/null 2>&1; then
...
quic_is_blocked() {
uci show firewall | grep -q "name='Block_UDP_80'" && uci show firewall | grep -q "name='Block_UDP_443'"
}
toggle_quic() {
if quic_is_blocked; then echo -e "\n${MAGENTA}Отключаем блокировку QUIC${NC}"
for RULE in Block_UDP_80 Block_UDP_443; do
while true; do
IDX=$(uci show firewall | grep "name='$RULE'" | cut -d. -f2 | cut -d= -f1 | head -n1)
[ -z "$IDX" ] && break; uci delete firewall.$IDX >/dev/null 2>&1; done; done
uci commit firewall >/dev/null 2>&1; /etc/init.d/firewall restart >/dev/null 2>&1
echo -e "${GREEN}Блокировка ${NC}QUIC ${GREEN}отключена${NC}\n"; read -p "Нажмите Enter..." dummy
else echo -e "\n${MAGENTA}Включаем блокировку QUIC${NC}"
uci add firewall rule >/dev/null 2>&1; uci set firewall.@rule[-1].name='Block_UDP_80' >/dev/null 2>&1
uci add_list firewall.@rule[-1].proto='udp' >/dev/null 2>&1; uci set firewall.@rule[-1].src='lan' >/dev/null 2>&1
uci set firewall.@rule[-1].dest='wan' >/dev/null 2>&1; uci set firewall.@rule[-1].dest_port='80' >/dev/null 2>&1
uci set firewall.@rule[-1].target='REJECT' >/dev/null 2>&1
uci add firewall rule >/dev/null 2>&1; uci set firewall.@rule[-1].name='Block_UDP_443' >/dev/null 2>&1
uci add_list firewall.@rule[-1].proto='udp' >/dev/null 2>&1; uci set firewall.@rule[-1].src='lan' >/dev/null 2>&1
uci set firewall.@rule[-1].dest='wan' >/dev/null 2>&1; uci set firewall.@rule[-1].dest_port='443' >/dev/null 2>&1
uci set firewall.@rule[-1].target='REJECT' >/dev/null 2>&1; uci commit firewall >/dev/null 2>&1
/etc/init.d/firewall restart >/dev/null 2>&1; echo -e "${GREEN}Блокировка ${NC}QUIC ${GREEN}включена${NC}\n"
read -p "Нажмите Enter..." dummy; fi; }
sys_menu() {
while true; do
...
echo -ne "${CYAN}Enter) ${GREEN}Выход в главное меню${NC}\n\n${YELLOW}Выберите пункт:${NC} " && read -r choiceMN
case "$choiceMN" in
...
echo -e "${CYAN}9) ${GREEN}Меню установки и настройки ${NC}DNS over HTTPS\n${CYAN}0) ${GREEN}Системная информация${NC}" ; echo -ne "${CYAN}Enter) ${GREEN}Выход${NC}\n\n${YELLOW}Выберите пункт:${NC} " && read choice
