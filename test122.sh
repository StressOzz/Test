...
# ==========================================
toggle_rkn_bypass() {
if awk 'NR==1{prev=$0; next} prev=="--filter-tcp=443" && $0=="--hostlist-exclude=/opt/zapret/ipset/zapret-hosts-user-exclude.txt"{found=1} {prev=$0} END{exit !found}' "$CONF" && grep -qE "#v[1-6]" "$CONF"; then
