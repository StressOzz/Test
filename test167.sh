...
ZAPRET_RESTART () { /etc/init.d/zapret stop; chmod +x /opt/zapret/sync_config.sh; /opt/zapret/sync_config.sh; /etc/init.d/zapret reload; /etc/init.d/zapret start; /etc/init.d/zapret reload; /etc/init.d/zapret restart }
...
ZAPRET_RESTART; sed -i 's|--hostlist-exclude=/opt/zapret/ipset/zapret-hosts-user-exclude.txt|--hostlist=/opt/zapret/ipset/zapret-hosts-user.txt|' "$CONF"; echo -e "${GREEN}Обход по спискам ${NC}РКН${GREEN} включен${NC}\n"; }
