...
SPL_VER="26.8.1.3"; TG_GO_VERSION="1.4.2"; TG_RS_VERSION="2.1.3"; BYEDPI_LATEST_VER="0.17.3"
...
95.182.120.241 accounts.scdn.co gew1-dealer.spotify.com open-exp.spotifycdn.com www-growth.scdn.co"
...
get_ver() {
    ...
    RESULT=$(curl -sIL --connect-timeout 3 --max-time 4 --retry 1 -w "%{url_effective}" -o /dev/null "$URL" 2>/dev/null)

    if [ $? -ne 0 ] || [ -z "$RESULT" ]; then
        echo -e "$NAME: ${RED}ошибка получения версии${NC}"
        return 1
    fi
...
TMP_VER_TG_MT="/tmp/tg_ws_proxy_MTp_ver"
TMP_VER_TG_GO="/tmp/tg_ws_proxy_GO_ver"
...
sync_ntp() {
    ...
    if command -v ntpd >/dev/null 2>&1; then
        ntpd -n -q -p 0.openwrt.pool.ntp.org -p 1.openwrt.pool.ntp.org >/dev/null 2>&1
    else
...
extract_yv_block() {
    awk '
        /^#Yv[0-9]/ { found=1 }
        found { print; if ($0 == "--new") exit }
    ' "$1"
}

extract_gv_block() {
    awk '
        /^#Gv[0-9]/ { found=1; print; next }
        found {
            if ($0 ~ /^#/ || $0 == "'"'"'") exit
            print
        }
...
auto_apply_best_strategy() {
    ...
    : > "$AUTO_LOG"
    {
        ...
        STR_FILE_AUTO="$TMP_SF/str_auto.txt"
        TEMP_FILE_AUTO="$TMP_SF/str_temp_auto.txt"
        : > "$STR_FILE_AUTO"
        cp "$CONF" "$AUTO_BACK"
        ...
        prepare_urls || {
            echo "Не удалось получить список для теста"
            mv -f "$AUTO_BACK" "$CONF"
            exit 1
        }
        URLS="$(cat "$OUT_DPI")"
        ...
        echo "$LINES" | while read -r START; do
            ...
            NAME=$(head -n1 "$TEMP_FILE_AUTO"); NAME="${NAME#\#}"
...
!skip {print}' "$CONF" > "${CONF}.tmp" && mv "${CONF}.tmp" "$CONF"
            ...
            ZAPRET_RESTART
            OK=0
            LOG_TMP="/tmp/zapret_log_auto_${CUR}"
            : > "$LOG_TMP"
            check_all_urls
...
auto_best_cron_line() { grep -F "$AUTO_CRON_CMD" "$CRON_FILE" 2>/dev/null | head -n1; }
LINECR=$(auto_best_cron_line)
HOURC=$(echo "$LINECR" | awk '{print $2}')
    ...
    echo -e "${CYAN}7) ${GREEN}Удалить результаты теста и лог${NC}"
        ...
        case "$choiceAB" in
            ...
            3) if [ "$RUNNING" = "1" ]; then
                   ...
                   stop_auto_best
                   echo -e "${GREEN}Автоподбор в фоне остановлен!${NC}\n"
                   PAUSE
               else
                   run_auto_best_background
               fi ;;
...
else clear; register_warp || continue; choose_endpoint || continue; create_warp_iface || continue; WARP_TO_ROOT; register_in_splify; restart_splify; echo -e "\nWARP ${GREEN}изменён!${NC}\n"; fi; PAUSE ;; 4) if [ -z "$SPL_INST_VER" ]; then echo -e "\nsplify ${RED}не установлен!${NC}\n"; else register_in_splify; restart_splify; fi; echo; PAUSE ;; *) return ;; esac; done; }
...
echo -e "${CYAN}8) ${GREEN}Удалить ${NC}fake $(stat_del)\n${CYAN}Enter) ${GREEN}Выход в меню настройки Discord${NC}"; echo -ne "\n${YELLOW}Выберите пункт:${NC} "; read -r choiceF; case "$choiceF" in 1) new_fileD="stun.bin";;
2) new_fileD="stun2.bin";; 3) new_fileD="quic_initial_4pda.to.bin";; 4) new_fileD="quic_initial_tencent_com.bin";; 5) new_fileD="quic_initial_dbankcloud_ru.bin";; 6) new_fileD="quic_initial_www_google_com.bin";; 7) new_fileD="quic_initial_steamcommunity_com.bin";;
8) new_fileD="DELETE";; *) return;; esac; [ "$new_fileD" = "DELETE" ] && echo -e "\n${CYAN}Удаляем ${NC}fake" || echo -e "\n${CYAN}Устанавливаем${NC} ${new_fileD}"; if [ "$new_fileD" = "DELETE" ]; then
...
echo -en "${CYAN}Enter) ${GREEN}Выход в меню стратегий${NC}\n\n${YELLOW}Выберите пункт:${NC} "; read -r choiceF; case "$choiceF" in 1) new_file="stun.bin";; 2) new_file="stun2.bin";; 3) new_file="quic_initial_4pda.to.bin";; 4) new_file="quic_initial_tencent_com.bin";;
5) new_file="quic_initial_dbankcloud_ru.bin";; 6) new_file="quic_initial_www_google_com.bin";; 7) new_file="quic_initial_steamcommunity_com.bin";; *) return;; esac; echo -e "\n${CYAN}Устанавливаем fake${NC} ${new_file}"
...
find "$BASE" -type f -name 'general*.bat' ! -name 'general (ALT5).bat' | while read -r F; do MATCH=$(grep -E '^--filter-udp=19294-19344,50000-50100|^--filter-tcp=2053,2083,2087,2096,8443|^--filter-tcp=443 --hostlist="%LISTS%list-google.txt"|^--filter-tcp=80,443 --hostlist="%LISTS%list-general.txt"' "$F")
[ -z "$MATCH" ] && continue; NAME=$(basename "$F" .bat); { echo "#$NAME"; echo "$MATCH" | sed 's/--/\n--/g' | sed '/^$/d' | sed 's/[[:space:]]*$//'; echo; } >> "$OUT"; done
sed -i 's|"%BIN%tls_clienthello_www_google_com.bin"|/opt/zapret/files/fake/tls_clienthello_www_google_com.bin|g' "$OUT"; sed -i '/--hostlist="%LISTS%list-general.txt"/d' "$OUT"; sed -i '/--hostlist="%LISTS%list-general-user.txt"/d' "$OUT"
sed -i '/--ipset-exclude="%LISTS%ipset-exclude.txt"/d' "$OUT"; sed -i '/--ipset-exclude="%LISTS%ipset-exclude-user.txt"/d' "$OUT"; sed -i '/--hostlist-exclude="%LISTS%list-exclude-user.txt"/d' "$OUT"
sed -i 's|"%LISTS%list-exclude.txt"|/opt/zapret/ipset/zapret-hosts-user-exclude.txt|g' "$OUT"; sed -i 's/--new[[:space:]]\^/--new/g' "$OUT"; sed -i 's|"%LISTS%list-google.txt"|/opt/zapret/ipset/zapret-hosts-google.txt|g' "$OUT"
...
9) doh_install && setup_doh "$doh_CLF" "Cloudflare";;
0) doh_install && setup_doh "$doh_GOO" "Google";;

10) doh_install && setup_doh "$doh_def" "настройки по умолчанию";;
...
then DOH_STATUS="dns.malw.link (CloudFlare)"; elif grep -q "dns.malw.link" "$fileDoH"; then DOH_STATUS="dns.malw.link"
elif grep -q "dns.mafioznik.xyz" "$fileDoH"; then DOH_STATUS="dns.mafioznik.xyz"

elif grep -q "cloudflare-dns.com" "$fileDoH" && grep -q "dns.google" "$fileDoH"; then DOH_STATUS="по умолчанию"
elif grep -q "cloudflare-dns.com" "$fileDoH"; then DOH_STATUS="Cloudflare"
elif grep -q "dns.google" "$fileDoH"; then DOH_STATUS="Google"


elif grep -q "dns.astracat.ru" "$fileDoH"; then DOH_STATUS="dns.astracat.ru"
elif grep -q "dns.nullsproxy.com" "$fileDoH"; then DOH_STATUS="dns.nullsproxy.com"; else DOH_STATUS="установлен"; fi; }
...
doh_CLF=$(printf "%s\n" "" "config https-dns-proxy" "	option resolver_url 'https://cloudflare-dns.com/dns-query'" "	option bootstrap_dns '1.1.1.1,1.0.0.1,2606:4700:4700::1111,2606:4700:4700::1001'")
doh_GOO=$(printf "%s\n" "" "config https-dns-proxy" "	option resolver_url 'https://dns.google/dns-query'" "	option bootstrap_dns '8.8.8.8,8.8.4.4,2001:4860:4860::8888,2001:4860:4860::8844'")
...
echo -e "${CYAN}6) ${GREEN}Меню автоподбора стратегий по расписанию${NC}"
...
1) TEST_STRATEGY_MENU;;
2) check_current_strategy;; 
3) run_test_by_domain;; 
4) auto_stryou;;
5) TEST_CUSTOM;;
6) AUTO_BEST_MENU;;
0) show_single_result "$RES_CUSTOM";;
7) show_domain_results;;
8) show_test_results;;
9) rm -f /opt/zapret/tmp/results*; echo -e "\n${GREEN}Результаты тестирования удалены!${NC}\n"; PAUSE;;

*) break;; esac; done; }

TEST_STRATEGY_MENU() {
        echo -e "\n${MAGENTA}Выберите стратегии для тестирования${NC}"
        echo -e "${CYAN}1) ${GREEN}Тестировать стратегии ${NC}v"
        echo -e "${CYAN}2) ${GREEN}Тестировать стратегии ${NC}Flowseal"
        echo -e "${CYAN}3) ${GREEN}Тестировать ${NC}v${GREEN} и ${NC}Flowseal${GREEN} стратегии${NC}"
        echo -e "${CYAN}Enter) ${GREEN}Выход в меню тестирования стратегий${NC}\n"
...
echo -e "\n${GREEN}===== Настройки Zapret =====${NC}"; if [ -f /etc/init.d/zapret ]; then INFO_ZPR; TCP_VAL=$(grep -E "^[[:space:]]*option NFQWS_PORTS_TCP[[:space:]]+'" "$CONF" | sed "s/.*'\(.*\)'.*/\1/"); UDP_VAL=$(grep -E "^[[:space:]]*option NFQWS_PORTS_UDP[[:space:]]+'" "$CONF" | sed "s/.*'\(.*\)'.*/\1/")
...
install_TG_RS() {
    ...
    CUR_VER_RS=""
    [ -s "$BIN_VER_RS" ] && CUR_VER_RS="$(cat "$BIN_VER_RS")"
    ...
    # Останавливаем сервис перед заменой бинарника, если он уже был установлен
    ...
    # Секрет нужен только для ПЕРВОГО создания init-скрипта.
    # Если SECRET уже задан (например, из меню) - используем его, иначе генерируем.
    [ -z "$SECRET" ] && SECRET="$(head -c16 /dev/urandom | hexdump -e '16/1 "%02x"')"

    # Если init-скрипта ещё нет — создаём его (шаблон 1 в 1 как в оригинале).
...
delete_TG_GO() {
    echo -e "\n${MAGENTA}Удаляем TG WS Proxy SOCKS5${NC}"

    /etc/init.d/tg-ws-proxy-go stop >/dev/null 2>&1
    /etc/init.d/tg-ws-proxy-go disable >/dev/null 2>&1

    rm -f "$BIN_PATH_GO" "$INIT_PATH_GO" "$BIN_VER_GO"

    echo -e "TG WS Proxy SOCKS5 ${GREEN}удалён!${NC}\n"
    PAUSE
}
...
install_TG_GO() {
    ...
    CUR_VER_GO=""
    [ -s "$BIN_VER_GO" ] && CUR_VER_GO="$(cat "$BIN_VER_GO")"
...
# УСТАНОВКА MTProto (без изменений - у вас там уже сравнение версий через opkg/apk)
...
menu_TG() {
    while true; do
        ...
        if [ -z "$INSTALLED_VER_MT" ]; then
            MT_ACTION="install"
        elif [ "$INSTALLED_VER_MT" != "$TG_MTProto" ]; then
            MT_ACTION="update"
        else
            MT_ACTION="installed"
        fi
        ...
        pidof tg-ws-proxy >/dev/null 2>&1 &&
            TGSTATUS="${TGSTATUS:+$TGSTATUS/}${NC}MTProto${GREEN}"

        pidof tg-ws-proxy-rs >/dev/null 2>&1 &&
        ...
        # Настройки показываем по факту установки (файлы на месте),
        # а не только если процесс жив именно в момент отрисовки меню.
        if [ "$GO_ACTION" != "install" ] && [ -f "$BIN_PATH_GO" ] && [ -f "$INIT_PATH_GO" ]; then

            echo -e "\n${YELLOW}Настройки ${CYAN}TG WS Proxy SOCKS5${YELLOW}:${NC}"
            if pidof tg-ws-proxy-go >/dev/null 2>&1; then
                echo -e "${YELLOW}Статус:${NC} ${GREEN}запущен${NC}"
            else
                echo -e "${YELLOW}Статус:${NC} ${RED}не запущен${NC}"
            fi
        ...
        if [ "$RS_ACTION" != "install" ] && [ -f "$BIN_PATH_RS" ] && [ -f "$INIT_PATH_RS" ]; then

            SECRET_IN_RS="$(
                sed -n 's/.*--secret[[:space:]]*\([0-9a-fA-F]\{32\}\).*/\1/p' \
                "$INIT_PATH_RS"
            )"

            echo -e "\n${YELLOW}Настройки ${CYAN}TG WS Proxy Rust${YELLOW}:${NC}"
            if pgrep -f tg-ws-proxy-rs >/dev/null 2>&1; then
                echo -e "${YELLOW}Статус:${NC} ${GREEN}запущен${NC}"
            else
                echo -e "${YELLOW}Статус:${NC} ${RED}не запущен${NC}"
            fi
        ...
        if [ "$MT_ACTION" != "install" ] && [ -f "/etc/init.d/tg-ws-proxy" ]; then

            SECRET_CONF="$(
                grep '^SECRET=' "$SECRET_FILE" 2>/dev/null |
                cut -d'=' -f2
            )"
        ...
        case "$choice" in
            ...
            4)
                ...
                for s in /etc/init.d/tg-ws*; do
                    [ -e "$s" ] || continue
                    "$s" stop >/dev/null 2>&1
                    "$s" disable >/dev/null 2>&1
                done

                $DELETE tg-ws* >/dev/null 2>&1
...
BYEDPI_SAVE_DNS_STATE() {
    if [ ! -f "$BYEDPI_DNS_BACKUP" ]; then
        ...
        if [ -n "$DNS_LOCALUSE" ]; then
            echo "$DNS_LOCALUSE" > "$BYEDPI_DNS_BACKUP"
        else
            echo "__UNSET__" > "$BYEDPI_DNS_BACKUP"
        fi
    fi
}
...
BYEDPI_RESTORE_DNS_STATE() {
    ...
    DNS_LOCALUSE="$(cat "$BYEDPI_DNS_BACKUP" 2>/dev/null)"

    echo -e "${CYAN}Восстанавливаем настройку локального DNS${NC}"

    if [ "$DNS_LOCALUSE" = "__UNSET__" ]; then
        uci delete dhcp.@dnsmasq[0].localuse >/dev/null 2>&1
    else
        uci set dhcp.@dnsmasq[0].localuse="$DNS_LOCALUSE"
    fi
...
BYEDPI_INSTALL() {
    ...
    echo -e "${CYAN}Скачиваем ${NC}$BYEDPI_FILE"
...
EOF
    ...
    echo -e "${CYAN}Запускаем ${NC}NetShift"
    netshift enable >/dev/null 2>&1
...
PODKOP_menu() {
    ...
    while true; do
        ...
        echo -e "${MAGENTA}Меню NetShift${NC}\n"
...
if pkg_is_installed byedpi; then
    ...
    echo -e "${YELLOW}Стратегия ByeDPI:${NC} $CURRENT_STRATEGY"
        ...
        if ! pkg_is_installed netshift; then
            ...
            echo -e "\n${CYAN}1) ${GREEN}Обновить ${NC}NetShift"
        else
            echo -e "\n${CYAN}1) ${GREEN}Удалить ${NC}NetShift"
        fi
                ...
                if pkg_is_installed amneziawg-tools &&
                   ...
                   pkg_is_installed kmod-amneziawg; then
                    ...
                    if ! pkg_is_installed splify; then
                        ...
                        echo -e "AmneziaWG ${YELLOW}используется в ${NC}splify\n"
                        PAUSE
                    fi
                else
                    install_AWG
...
if [ -f "$CONFIGPATH" ]; then
    if grep -Fq 'name: Google_ai' "$CONFIGPATH"; then
        echo -e "${YELLOW}Используется список: ${NC}ITDog"

    elif grep -Fq 'name: Meta (WA+FB+Instagram)' "$CONFIGPATH"; then
        echo -e "${YELLOW}Используется список: ${NC}Internet Helper #2"

    elif grep -Fq 'url: https://sw.ext.io/ipset/ipset_cf.list' "$CONFIGPATH"; then
        echo -e "${YELLOW}Используется список: ${NC}Internet Helper #1"
    fi
fi
