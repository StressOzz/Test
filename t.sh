...
# WARP registration API (обход блокировки домена: ходим по IP, но с доменным SNI/Host)
API_PATH="v0i1909051800"
API_HOST="api.cloudflareclient.com"
API_IPS="162.159.137.105 162.159.138.105"

ins() {
    method="$1"; path="$2"; shift 2

    for ip in $API_IPS; do
        out="$(curl -s --connect-timeout 5 --max-time 20 \
            --resolve "${API_HOST}:443:${ip}" \
            -H "User-Agent: okhttp/3.12.1" \
            -H "Content-Type: application/json" \
            -X "$method" "https://${API_HOST}/${API_PATH}/${path}" "$@")"

        # Для POST reg ожидаем .result.id, для PATCH может быть .success
        echo "$out" | jq -e '.result.id? // .success? // empty' >/dev/null 2>&1 && {
            echo "$out"
            return 0
        }
