...
ZAPRET_RESTART; echo -e "${GREEN}Обход по спискам ${NC}РКН${GREEN} включен${NC}\n"; }
...
enable_rkn; fi; read -p "Нажмите Enter..." dummy </dev/tty; return; fi; if grep -qE "^#v|^#general" "$CONF" && grep -q -- "--filter-" "$CONF" && grep -qP '(?<=--filter-.*\n)--hostlist-exclude=/opt/zapret/ipset/zapret-hosts-user-exclude.txt' "$CONF"; then enable_rkn; read -p "Нажмите Enter..." dummy </dev/tty
