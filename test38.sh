...
[ -n "$DOH_STATUS" ] && echo -e "${YELLOW}DNS over HTTPS: ${GREEN}$DOH_STATUS${NC}\n"; echo -e "${CYAN}1)${GREEN} $doh_st ${NC}DNS over HTTPS"; echo -e "${CYAN}2)${GREEN} Настроить ${NC}Comss DNS"
echo -e "${CYAN}3)${GREEN} Настроить ${NC}Xbox DNS"; echo -e "${CYAN}4)${GREEN} Настроить ${NC}dns.malw.link"; echo -e "${CYAN}5)${GREEN} Настроить ${NC}dns.malw.link (CloudFlare)"
echo -ne "${CYAN}Enter) ${GREEN}Выход в главное меню${NC}\n\n${YELLOW}Выберите пункт:${NC} "; read -r choiceDOH; case "$choiceDOH" in 1) D_o_H ;; 2) doh_install && setup_doh "$doh_comss" "Comss.one DNS" ;;
...
6) doh_install && setup_doh "$doh_def" "Настройки по умолчанию" ;; *) return ;; esac; done; }
setup_doh() { local config="$1"; local name="$2"; echo -e "\n${MAGENTA}Настраиваем DNS over HTTPS${NC}\n${CYAN}$name\nПрименяем новые настройки${NC}"; rm -f "$fileDoH"
