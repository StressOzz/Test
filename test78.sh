...
# =========================================
ZAPRET_MANAGER_VERSION="8.1"; ZAPRET_VERSION="72.20260115"; STR_VERSION_AUTOINSTALL="v6"
...
RKN_Check() {
    local userlist="/opt/zapret/ipset/zapret-hosts-user.txt"
    local enabled=0

    if [ -f "$userlist" ] && [ "$(wc -c < "$userlist")" -gt 1800000 ]; then
        if grep -q -- "--hostlist=$userlist\|--filter-tcp=443 <HOSTLIST>" "$CONF"; then
            enabled=1
        fi
    fi
...
show_current_strategy() {
    ...
    ver=$(grep -m1 -o '#v[0-9]\+' "$CONF" | head -n1 | tr -d '#')
    yv_ver=$(grep -m1 -o '#Yv[0-9]\+' "$CONF" | head -n1 | tr -d '#')
}
...
get_doh_status() {
    ...
    if grep -m1 -Fq "dns.comss.one" "$fileDoH"; then
        DOH_STATUS="Comss DNS"
    elif grep -m1 -Fq "xbox-dns.ru" "$fileDoH"; then
        DOH_STATUS="Xbox DNS"
    elif grep -m1 -Fq "5u35p8m9i7.cloudflare-gateway.com" "$fileDoH"; then
        DOH_STATUS="dns.malw.link (CloudFlare)"
    elif grep -m1 -Fq "dns.malw.link" "$fileDoH"; then
        DOH_STATUS="dns.malw.link"
    elif grep -m1 -Fq "dns.mafioznik.xyz" "$fileDoH"; then
        DOH_STATUS="dns.mafioznik.xyz"
    elif grep -m1 -Fq "dns.astracat.ru" "$fileDoH"; then
        DOH_STATUS="dns.astracat.ru"
    else
