...
if { [ -s "/opt/zapret/tmp/results_flowseal.txt" ] || [ -s "/opt/zapret/tmp/results_versions.txt" ]; }; then echo -e "${CYAN}0) ${GREEN}Результат тестирования стратегий${NC}"; fi
...
show_test_results() {
    ...
    TMP_RES="/tmp/zapret_results_all.$$"
