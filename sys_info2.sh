...
clear
echo -e "${GREEN}===== Model =====${NC}"; cat /tmp/sysinfo/model; awk -F= '/DISTRIB_ARCH/ {gsub(/'\''/, ""); print ""$2} /DISTRIB_TARGET/ {gsub(/'\''/, ""); print ""$2}' /etc/openwrt_release;
echo -e "\n${GREEN}===== OpenWrt =====${NC}"; awk -F= '/DISTRIB_DESCRIPTION/ {gsub(/'\''|OpenWrt /,""); print $2}' /etc/openwrt_release;
...
echo -e "\n${GREEN}===== GitHub API Rate Limit =====${NC}"; echo -e "Limit remaining: $(curl -s https://api.github.com/rate_limit | sed -n 's/.*\"remaining\": \([0-9]*\).*/\1/p' | head -1)\n"
