...
hosts_clear() { while IFS= read -r line; do [ -n "$line" ] && sed -i "\|$line|d" /etc/hosts; done <<< "$(echo -e "$HOSTS_LIST")"; /etc/init.d/dnsmasq restart >/dev/null 2>&1; }
hosts_add () { while IFS= read -r line; do [ -n "$line" ] && sed -i "\$a\\$line" /etc/hosts; done <<< "$(echo -e "$HOSTS_LIST")"; /etc/init.d/dnsmasq restart >/dev/null 2>&1; }
# ==========================================
# Доступ из браузера
# ==========================================
