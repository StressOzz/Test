...
uci set network.$IF_NAME.proto=$PROTO; uci set network.$IF_NAME.device=$DEV_NAME; uci commit network; fi; echo -en "${YELLOW}Перезапускаем сеть! Подождите...${NC}"; /etc/init.d/network restart >/dev/null 2>&1; echo -e "${YELLOW}Необходимо в ${NC}LuCI${YELLOW} в интерфейс ${NC}AWG${YELLOW} загрузить файл ${NC}*.conf${YELLOW}:${NC}"
...
1) $Z_ACTION_FUNC;; 
# 2) pgrep -f /opt/zapret >/dev/null 2>&1 && stop_zapret || start_zapret;;  ${CYAN}2) ${GREEN}$str_stp_zpr ${NC}Zapret
2) menu_str;; 3)TEST_menu;; 4) menu_TG;; 
5) PODKOP_menu ;;
6) DoH_menu;; 7) Discord_menu;; 8) menu_hosts;; 9) zapret_key;; 0) sys_menu;; *) echo; exit 0;; esac; }; while true; do show_menu; done
