...
echo -e "\nPodkop Evolution ${GREEN}$( [ "$ACTION" = "install" ] && echo "установлен" || echo "обновлён" )!${NC}\n"; PAUSE; else echo -e "\n${MAGENTA}Удаляем Podkop${NC}"
$DELETE luci-i18n-podkop-ru >/dev/null 2>&1; $DELETE luci-app-podkop >/dev/null 2>&1; $DELETE podkop >/dev/null 2>&1; rm -rf /etc/config/podkop* /usr/bin/podkop >/dev/null 2>&1; echo -e "Podkop ${GREEN}удалён!${NC}\n"; PAUSE; fi; }
# УСТАНОВКА AWG
...
if uci -q get network.AWG >/dev/null; then echo -e "${YELLOW}Интерфейс AWG: ${GREEN}установлен${NC}"; else echo -e "${YELLOW}Интерфейс AWG: ${RED}не установлен${NC}"; fi;
