...
# РКН список ВКЛ / ВЫКЛ
# ==========================================
toggle_rkn_bypass(){ if awk 'NR==1{prev=$0; next} prev=="--filter-tcp=443" && $0=="--hostlist-exclude=/opt/zapret/ipset/zapret-hosts-user-exclude.txt"{found=1} {prev=$0} END{exit !found}' "$CONF" && grep -qE "#v[1-6]" "$CONF"
then echo -e "\n${MAGENTA}Включаем списки ${NC}РКН"; [ -f /opt/zapret/ipset/zapret-hosts-user.txt ] && cp /opt/zapret/ipset/zapret-hosts-user.txt /opt/hosts-user.txt; chmod +x /opt/zapret/sync_config.sh; /opt/zapret/sync_config.sh; /etc/init.d/zapret restart >/dev/null 2>&1
sed -i '/--filter-tcp=443/{n;s|--hostlist-exclude=/opt/zapret/ipset/zapret-hosts-user-exclude.txt|--hostlist=/opt/zapret/ipset/zapret-hosts-user.txt|}' "$CONF"; curl -fsSL https://raw.githubusercontent.com/IndeecFOX/zapret4rocket/refs/heads/master/extra_strats/TCP/RKN/List.txt -o /opt/zapret/ipset/zapret-hosts-user.txt
...
sed -i '/--filter-tcp=443/{n;s|--hostlist=/opt/zapret/ipset/zapret-hosts-user.txt|--hostlist-exclude=/opt/zapret/ipset/zapret-hosts-user-exclude.txt|}' "$CONF"; [ -f /opt/hosts-user.txt ] && cp /opt/hosts-user.txt /opt/zapret/ipset/zapret-hosts-user.txt || : > /opt/zapret/ipset/zapret-hosts-user.txt
